 ...
 *
 * ─── Por qué vive en `lib/` y no en la ruta ───────────────────────────────────
 *
 * Hasta el 2026-09-07 esto era el cuerpo de `app/api/admin/cases/[id]/pdf/
 * route.tsx`, y era el ÚNICO lugar donde existía. Ahora lo piden tres:
 * el back-office, el tab Documentos y el portal legal —cada uno con su propio
 * control de acceso— y una ruta de Next **no puede exportar nada que no sea un
 * verbo HTTP** (`next build` lo rechaza; `tsc` no lo ve). Así que el documento
 * se arma acá y cada ruta pone su guard y llama a `respuestaIntakePdf`.
 *
 * ⚠️ Esta función NO autoriza a nadie. Recibe un `caseId` y devuelve el PDF de
 * ese caso. Quien la llama decide si el que pide tiene derecho — es el mismo
 * criterio de `lib/lawyer-access.ts`: un helper que además autoriza es un helper
...
/**
 * El intake de un caso, listo para devolver desde una ruta.
 *
 * @param id        el caso. **Ya autorizado por quien llama** (ver el docblock
 *                  de arriba): acá no se chequea nada.
 * @param descargar `true` fuerza la descarga (`attachment`); `false` —el
 *                  default— lo abre en el visor del navegador (`inline`), que
 *                  es lo que necesita el `<iframe>` de la vista previa y lo que
 *                  permite imprimir sin bajar el archivo primero.
 */
export async function respuestaIntakePdf(
  id: string,
  { descargar = false }: { descargar?: boolean } = {},
  ...
  return new NextResponse(new Uint8Array(buffer), {
    ...
    headers: {
      ...
      // `inline` seguía siendo el único modo, así que el botón "Descargar" del
      // tab abría el visor en vez de bajar el archivo.
