...
export async function sendPasswordResetEmail(email: string): Promise<{ error?: string }> {
  try {
    ...
    const { error: emailError } = await resend.emails.send({
      ...
      to: email.trim(),
      subject: 'Recuperá tu contraseña — Neural Trainer Gym',
...
<table width="100%" style="max-width:480px" cellpadding="0" cellspacing="0">
  ...
  <tr><td style="background:rgba(5,20,25,0.95);border:1px solid rgba(0,200,180,0.2);border-radius:8px;padding:32px">
    <p style="color:#c8f0eb;font-size:15px;line-height:1.7;margin:0 0 20px">Recibimos una solicitud para restablecer tu contraseña.</p>
    <p style="color:#c8f0eb;font-size:15px;line-height:1.7;margin:0 0 28px">Hacé clic en el botón para crear una nueva contraseña. El enlace es válido por <strong>1 hora</strong>.</p>
    ...
    <p style="color:rgba(0,200,180,0.45);font-size:11px;text-align:center;line-height:1.7;margin:0">Si no solicitaste este cambio, podés ignorar este correo.<br><br>Si el botón no funciona, copiá este enlace:<br><span style="word-break:break-all;color:rgba(0,200,180,0.6)">${link}</span></p>
  </td></tr>
</table>
</td></tr></table>
</body></html>`;
