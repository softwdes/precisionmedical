...
/**
 * B.22 — Firma del Lien Médico (abogado) — client component
 */
...
export default function SignLienClient({ initialSignerName, alreadySigned: initAlreadySigned, patientSigned }: Props) {
  ...
  const [alreadySigned, setAlreadySigned] = useState(initAlreadySigned);
  ...
  const stopDrawing = () => { isDrawing.current = false; lastPoint.current = null; };
  ...
  const handleSubmit = async () => {
    ...
    try {
      const res = await fetch(`/api/cases/${caseId}/sign`, {
        ...
        body:    JSON.stringify({ signerName: signerName.trim(), signerEmail: signerEmail.trim() || undefined, signatureSvg }),
      });
      ...
      <div style={{ padding: '24px', maxWidth: 720, margin: '0 auto', display: 'flex', flexDirection: 'column', gap: 20 }}>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 12 }}>
          ...
          <div style={{
            ...
            background: patientSigned ? 'rgba(52,211,153,0.06)' : 'rgba(255,255,255,0.02)',
            ...
            <div style={{ display: 'flex', alignItems: 'center', gap: 7, marginBottom: 6 }}>
              <span style={{
                ...
                background: patientSigned ? '#34d399' : 'transparent',
                border: patientSigned ? 'none' : '1px solid rgba(255,255,255,0.25)',
        ...
        <div style={{
          ...
          <canvas
            ...
            style={{ width: '100%', height: 180, cursor: 'crosshair', display: 'block', background: 'rgba(0,0,0,0.20)', touchAction: 'none' }}
          />
          ...
          <button
            ...
            style={{
              ...
              color: (!hasSig || !signerName.trim() || !agreed || saving)
                ? 'rgba(255,255,255,0.30)' : '#fff',
              border: 'none', cursor: saving ? 'wait' : 'pointer',
              boxShadow: (!hasSig || !signerName.trim() || !agreed || saving) ? 'none' : '0 4px 14px rgba(244,63,94,0.35)',
              transition: 'all 0.15s',
