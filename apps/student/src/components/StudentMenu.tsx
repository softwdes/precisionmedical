...
type ModalType = 'perfil' | 'password' | null;
...
const MODAL_BODY: React.CSSProperties = { padding: '24px', display: 'flex', flexDirection: 'column', gap: '14px' };
...
const ERR_BOX: React.CSSProperties = {
  ...
  border: '1px solid rgba(255,80,80,0.3)', borderRadius: '6px',
  fontSize: '13px', color: '#ff6b6b',
};
const ITEM: React.CSSProperties = {
  display: 'flex', alignItems: 'center', gap: '10px',
  padding: '8px 10px', borderRadius: '6px', fontSize: '13px',
  color: 'var(--fg-base)', width: '100%', background: 'none',
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      const { data } = await supabase
        .from('students')
        .select('full_name, experience_level, goals')
        .eq('user_id', user.id)
        .single();
      if (data) {
  ...
  function handlePasswordSubmit(e: React.FormEvent<HTMLFormElement>) {
    ...
    startTransition(async () => {
      ...
      if (res?.error) {
        ...
        setModal(null);
        setToastMsg('Contraseña actualizada correctamente');
      }
    });
  }
    ...
    <>
        ...
        {dropdownOpen && (
          <div style={{
            ...
            <div style={{ padding: '12px 16px', borderBottom: '1px solid var(--border)' }}>
              ...
              <div style={{ fontSize: '11px', color: 'var(--fg-muted)', marginTop: '2px' }}>Alumno</div>
            </div>
            <div style={{ padding: '6px' }}>
              <button onClick={() => openModal('perfil')} style={ITEM}>
        ...
        <div style={OVERLAY} onClick={() => setModal(null)}>
          <div style={MODAL} onClick={e => e.stopPropagation()}>
            <div style={MODAL_HEAD}>
              <div>
                <div style={{ fontWeight: 700, fontSize: '16px', color: 'var(--accent)' }}>Mi Perfil</div>
                <div style={{ fontSize: '12px', color: 'var(--fg-muted)', marginTop: '2px' }}>Información de tu cuenta</div>
              </div>
            ...
            <div style={MODAL_BODY}>
              <div style={{ display: 'flex', flexDirection: 'column', gap: '12px' }}>
                {[
                  ...
                  { label: 'Nivel', value: LEVEL_LABELS[level] ?? level ?? '—' },
                ].map(({ label, value }) => (
                  <div key={label} style={{ display: 'flex', flexDirection: 'column', gap: '4px' }}>
                    <span style={{ fontSize: '11px', fontWeight: 600, color: 'var(--fg-muted)', textTransform: 'uppercase', letterSpacing: '0.07em' }}>{label}</span>
                    <span style={{ fontSize: '14px', color: 'var(--fg-base)' }}>{value || '—'}</span>
                  </div>
                ))}
                {goals.length > 0 && (
                  <div style={{ display: 'flex', flexDirection: 'column', gap: '4px' }}>
                    <span style={{ fontSize: '11px', fontWeight: 600, color: 'var(--fg-muted)', textTransform: 'uppercase', letterSpacing: '0.07em' }}>Objetivos</span>
                    <div style={{ display: 'flex', flexWrap: 'wrap', gap: '6px' }}>
                      {goals.map(g => (
                        <span key={g} style={{ fontSize: '12px', padding: '3px 10px', borderRadius: '99px', background: 'rgba(63,248,200,0.1)', color: 'var(--accent)', border: '1px solid rgba(63,248,200,0.25)' }}>
                          {GOAL_LABELS[g] ?? g}
                        </span>
                      ))}
                    </div>
        ...
        <div style={OVERLAY} onClick={() => !isPending && setModal(null)}>
          <div style={{ ...MODAL, maxWidth: '380px' }} onClick={e => e.stopPropagation()}>
            ...
            <form onSubmit={handlePasswordSubmit}>
              <div style={MODAL_BODY}>
                {passwordError && <div style={ERR_BOX}>{passwordError}</div>}
                <div className="form-group">
                  <label className="label" htmlFor="pw-new">Nueva contraseña</label>
                  <input id="pw-new" name="password" type="password" className="input" required minLength={8} placeholder="Mínimo 8 caracteres" autoComplete="new-password" disabled={isPending} />
                </div>
                <div className="form-group">
                  <label className="label" htmlFor="pw-confirm">Confirmar contraseña</label>
                  <input id="pw-confirm" name="confirm" type="password" className="input" required minLength={8} placeholder="Repite la nueva contraseña" autoComplete="new-password" disabled={isPending} />
                </div>
              </div>
              <div style={MODAL_FOOT}>
                <button type="button" onClick={() => setModal(null)} className="btn btn-outline" disabled={isPending}>Cancelar</button>
                <button type="submit" className="btn btn-primary" disabled={isPending}>{isPending ? 'Actualizando...' : 'Actualizar contraseña'}</button>
              </div>
            </form>
          </div>
        </div>
      ...
      {toastMsg && (
        <div style={{
          ...
          boxShadow: '0 8px 32px rgba(0,0,0,0.55)', fontSize: '14px',
          fontWeight: 500, color: 'var(--fg-base)', pointerEvents: 'none',
          transform: toastVisible ? 'translateY(0) scale(1)' : 'translateY(12px) scale(0.97)',
          opacity: toastVisible ? 1 : 0,
          transition: 'transform 0.3s cubic-bezier(0.34,1.56,0.64,1), opacity 0.3s ease',
        }}>
          <span style={{ width: '22px', height: '22px', background: 'var(--accent)', borderRadius: '50%', display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0 }}>
