...
const STRINGS = {
  es: {
    ...
    dlLabel: 'ID / Licencia de conducir',
...
const S = {
  ...
  /**
   * ⚠️ `fontSize: 16` NO es una decisión de diseño: es el piso que impone iOS.
   *
   * Safari de iPhone y iPad hace zoom AUTOMÁTICO al tocar un campo de texto
   * cuya letra mida menos de 16px, y se queda ahí: el paciente tiene que
   * alejar con los dedos después de cada campo. En un formulario de diez pasos
   * son unas cuarenta veces.
   *
   * Hasta el 17-sep esto no se notaba porque el `viewport` traía
   * `maximumScale: 1` y `userScalable: false`, que además de bloquear el zoom
   * del paciente apagaban el automático. Al sacar ese bloqueo —ver el
   * comentario en `app/layout.tsx`, se sacó para que la gente mayor pueda
   * agrandar la letra— el zoom automático quedó suelto. Estos dos campos
   * estaban en 15 y 14.
   *
   * O sea: los dos cambios van juntos o ninguno. Bajar de 16 acá reintroduce
...
export function IntakeWizard({
  ...
  const goNext = async (fromStep: Step) => {
    ...
    if (fromStep === 8) {
      ...
      /**
       * Faltan fotos: NO SE PUEDE SEGUIR. El paciente sube sus documentos sí o sí.
       *
       * ── Por qué esto volvió a ser una pared (Erick, 24-sep-2026) ───────────
       *
       * Entre el 17 y el 24 de septiembre acá hubo una salida: "No puedo
       * tomarlas ahora / las llevo a la clínica". Se construyó el día que una
       * paciente con iPhone quedó encerrada porque el botón para elegir la foto
       * de la galería **estaba roto y no hacía nada**, y el paso no la dejaba
       * avanzar. En ese momento la salida era lo único que evitaba perder el
       * formulario entero.
       *
       * Se saca por dos razones:
       *
       *  1. **Ese botón ya está arreglado** (`bf883c7a`). La causa real de "no
       *     puedo sacar la foto" era un bug nuestro, no el paciente. Con la
       *     galería funcionando, la salida dejó de tener a quién rescatar.
       *  2. **La midieron sus consecuencias**: de 34 intakes en una semana, 7
       *     terminaron sin alguna foto — 4 pacientes reales sin su selfie o su
       *     licencia, y la clínica enterándose de a uno, a mano.
       *
       * Erick: "el paciente sí o sí toma fotos a sus documentos". La salida que
       * se construyó pertenece al lado de la CLÍNICA —donde alguien del staff
       * puede decidir y hacerse cargo— y no al formulario que llena el paciente
       * solo en su casa.
       */
      if (faltan.length > 0) {
        setPhotoUploadError(
                        ...
                        {consents.authorizedPersons.map((p, i) => (
                          <div key={i}>
                            ...
                            <div style={{ display: 'flex', gap: 6, alignItems: 'center' }}>
                              <input type="text"
                                /* 16 y no 12: es un campo de texto y menos de
                                   16 hace que iOS acerque la pantalla sola al
                                   tocarlo. Ver la nota en `S.input`. */
                                style={{ ...S.input, padding: '8px 10px', flex: '2 1 120px', minWidth: 0 }}
                              ...
                              <select
                                style={{
                                  ...S.input, padding: '8px 10px', flex: '1 1 100px', minWidth: 0,
