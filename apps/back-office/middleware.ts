 ...
 *
 * Solo activo cuando STAGING_PASSWORD está definido en el entorno.
 * En local dev (sin esa variable) pasa todo sin restricción.
 *
 * Flujo:
 *   1. Cookie pm_stg presente y correcta → pasar
 *   2. POST /api/staging-auth → manejado por la route (excluido del matcher)
 *   3. Todo lo demás → retornar HTML de login (inline, sin depender de Next.js pages)
 */
...
export function middleware(req: NextRequest) {
  // Sin STAGING_PASSWORD configurado → desarrollo local, pasa todo
  if (!STAGING_PW) return NextResponse.next();

  // Cookie válida → pasa
  if (req.cookies.get(COOKIE)?.value === STAGING_PW) {
    return NextResponse.next();
  }

  // Callback URL para redirigir después del login
...
export const config = {
  // Excluir rutas que no necesitan protección: assets estáticos + auth handler
  matcher: [
