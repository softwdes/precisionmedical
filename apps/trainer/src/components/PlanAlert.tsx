...
// Update this number to your support WhatsApp
const WA_NUMBER = '51993859385';
const WA_MSG = encodeURIComponent('Hola, quiero renovar mi plan de Neural Trainer Gym.');
...
const PLAN_META: Record<string, { label: string; badge?: string }> = {
  basico:  { label: 'Básico' },
  vip:     { label: 'VIP', badge: 'Más popular' },
...
export default function PlanAlert({ planNombre, planActualNombre, diasRestantes, fechaVencimiento, planes, sessionKey }: Props) {
  const [show, setShow]         = useState(false);
  const [visible, setVisible]   = useState(false);
  const [showPlans, setShowPlans] = useState(false);

  useEffect(() => {
    const key = `precision_plan_alert_${sessionKey}`;
  ...
  const isCritical = diasRestantes <= 2;
  const urgColor   = isExpired || isCritical ? '#ef4444' : '#EF9F27';
  const urgBg      = isExpired || isCritical ? 'rgba(239,68,68,0.08)'  : 'rgba(239,159,39,0.08)';
  const urgBd      = isExpired || isCritical ? 'rgba(239,68,68,0.28)'  : 'rgba(239,159,39,0.28)';
  ...
  return (
    ...
    >
      <div
        style={{
          width: '100%', maxWidth: showPlans ? '520px' : '400px',
          background: '#0c0f0e',
          border: `1px solid ${showPlans ? 'rgba(255,255,255,0.1)' : urgBd}`,
          borderRadius: '14px', overflow: 'hidden',
          boxShadow: '0 28px 72px rgba(0,0,0,0.9)',
          transform: visible ? 'translateY(0) scale(1)' : 'translateY(12px) scale(0.97)',
          transition: 'transform 0.32s cubic-bezier(0.34,1.4,0.64,1), max-width 0.25s ease',
      ...
      >
        ...
        {!showPlans && (
          <>
            <div style={{ height: '3px', background: urgColor, opacity: 0.9 }} />
            <div style={{ padding: '24px' }}>
              <div style={{ display: 'flex', gap: '14px', alignItems: 'flex-start', marginBottom: '18px' }}>
                <div style={{
                  width: '42px', height: '42px', borderRadius: '50%', flexShrink: 0,
                  background: urgBg, border: `1px solid ${urgBd}`,
                  display: 'flex', alignItems: 'center', justifyContent: 'center',
                }}>
                  <svg viewBox="0 0 24 24" fill="none" stroke={urgColor} strokeWidth="2" strokeLinecap="round" width="20" height="20">
                ...
                <div>
                  <div style={{ fontWeight: 700, fontSize: '15px', color: '#E6EDEC', marginBottom: '5px' }}>
                    {isExpired ? 'Tu plan ha vencido' : `Tu plan vence en ${diasRestantes} día${diasRestantes !== 1 ? 's' : ''}`}
                  </div>
                  <div style={{ fontSize: '13px', color: '#6B7472', lineHeight: 1.55 }}>
                    {isExpired
                      ? `Tu plan ${planNombre} ha vencido. Renueva para recuperar acceso completo.`
                      : `Tu plan ${planNombre} vence el ${fmtDate(fechaVencimiento)}. Renueva para no perder acceso a tus alumnos y datos.`}
                  </div>
                </div>
              </div>
              ...
              {!isExpired && (
                <div style={{
                  ...
                  <span style={{ fontSize: '11px', color: '#6B7472', fontWeight: 600, textTransform: 'uppercase', letterSpacing: '0.07em' }}>
                    Días restantes
                  </span>
                  <span style={{ fontSize: '28px', fontWeight: 700, color: urgColor, fontFamily: 'var(--font-serif)', lineHeight: 1 }}>
                    {diasRestantes}
                  </span>
                </div>
              )}
              ...
              <div style={{ display: 'flex', gap: '10px' }}>
                <button
                  onClick={() => setShow(false)}
                  style={{
                    flex: 1, height: '42px', cursor: 'pointer', background: 'none',
                    border: '1px solid rgba(255,255,255,0.1)', borderRadius: 'var(--radius-sm)',
                    color: '#6B7472', fontFamily: 'var(--font-sans)', fontWeight: 600, fontSize: '13px',
                  }}
                >
                  Recordar luego
                </button>
                <button
                  ...
                  style={{
                    flex: 1, height: '42px', cursor: 'pointer', background: urgColor, border: 'none',
                    borderRadius: 'var(--radius-sm)', color: '#fff',
                    fontFamily: 'var(--font-sans)', fontWeight: 700, fontSize: '13px', letterSpacing: '0.04em',
                  }}
                >
                  Renovar plan
                </button>
        ...
        {showPlans && (
          <div style={{ padding: '20px' }}>
            ...
            <div style={{ marginBottom: '18px' }}>
              <div style={{ fontWeight: 700, fontSize: '15px', color: '#E6EDEC', marginBottom: '3px' }}>Planes disponibles</div>
              <div style={{ fontSize: '12px', color: '#6B7472' }}>Elige el plan que mejor se adapta a tu negocio</div>
            </div>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: '10px', marginBottom: '16px' }}>
              {planes.map(plan => {
                ...
                const cardBorder = isCurrent ? 'var(--accent)' : isPopular ? 'rgba(63,248,200,0.25)' : 'rgba(255,255,255,0.08)';
                ...
                return (
                  <div key={plan.id} style={{
                    ...
                    borderRadius: '10px', background: cardBg, overflow: 'hidden',
                  }}>
                    {/* Card header */}
                    <div style={{
                      display: 'flex', alignItems: 'center', justifyContent: 'space-between',
                      ...
                      <span style={{ fontWeight: 700, fontSize: '15px', color: isCurrent ? 'var(--accent)' : '#E6EDEC' }}>
                        ${plan.precio_mensual}<span style={{ fontWeight: 400, fontSize: '11px', color: '#6B7472' }}>/mes</span>
                      </span>
                    </div>
                      ...
                      {isCurrent ? (
                        ...
                        <a
                          ...
                          style={{
                            display: 'flex', alignItems: 'center', justifyContent: 'center',
                            height: '36px', borderRadius: 'var(--radius-sm)',
                            background: isPopular ? 'rgba(63,248,200,0.1)' : 'rgba(255,255,255,0.05)',
                            border: `1px solid ${isPopular ? 'rgba(63,248,200,0.3)' : 'rgba(255,255,255,0.1)'}`,
                            color: isPopular ? 'var(--accent)' : '#C8D5D3',
