...
export async function POST(
  ...
  const path = `${freelancerId}/bank-qr.${ext}`;

  const admin = createAdminClient();

  // Remove any existing QR files for this freelancer before uploading
  const { data: existing } = await admin.storage.from(BUCKET).list(freelancerId);
  if (existing?.length) {
    await admin.storage.from(BUCKET).remove(existing.map(f => `${freelancerId}/${f.name}`));
  }
