...
/**
 * Los tipos y el hook viven en `case-clinical-data.ts`.
 *
 * Se movieron ahí para que ESTE archivo se pueda diferir con `next/dynamic`:
 * `useCaseClinical` es un hook, `case-detail-client` lo llama en el nivel
 * superior de su render, y mientras vivieran en el mismo archivo el import del
 * hook arrastraba los cuatro tabs al bundle de trece rutas — aunque los tabs ya
 * se rindan condicionados por `activeTab`. El gate estaba bien puesto y no
 * servía de nada, porque había otra puerta al mismo módulo.
 *
 * Se RE-EXPORTAN para no romper a nadie que los importe de acá.
 */
export type {
  ClinicalTabProps, Visit, RxRow, LabRow, ClinicalPayload, CaseClinical,
} from './case-clinical-data';
export { useCaseClinical } from './case-clinical-data';

/** Y aparte para el uso INTERNO: un `export … from` re-exporta, no trae al alcance. */
import type {
  ClinicalTabProps, Visit, CaseClinical, RxRow, LabRow,
} from './case-clinical-data';
